use crate::tracking;
use crate::utils::{detect_package_manager, resolve_binary, strip_ansi};
...
pub fn run(args: &[String], verbose: u8) -> Result<()> {
    ...
    let mut cmd = match pm {
        "pnpm" => {
            let mut c = std::process::Command::new(resolve_binary("pnpm"));
            c.arg("exec").arg("--").arg("playwright");
            c
        }
        "yarn" => {
            let mut c = std::process::Command::new(resolve_binary("yarn"));
            c.arg("exec").arg("--").arg("playwright");
            c
        }
        _ => {
            let mut c = std::process::Command::new(resolve_binary("npx"));
